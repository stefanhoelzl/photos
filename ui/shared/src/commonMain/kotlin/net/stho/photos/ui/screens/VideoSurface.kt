 ...
 * implementation lives in an app module — `:app:media` over libvlc for both Linux roots,
...
/**
 * Play/pause asked for from outside the player — Space on the desktop (§11) — for whichever video
 * or Live Photo is open. A count of presses rather than an event, so a surface composed after a
 * press does not act on it: it acts on presses beyond the count it first saw.
 */
@Stable
public class PlayToggle {
    public var presses: Int by mutableIntStateOf(0)
        private set
...
/** Provided by a root that has a key for it. The default is one nobody presses. */
public val LocalPlayToggle: androidx.compose.runtime.ProvidableCompositionLocal<PlayToggle> =
    staticCompositionLocalOf { PlayToggle() }
